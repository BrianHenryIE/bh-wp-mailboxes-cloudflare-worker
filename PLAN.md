...
| Multiple ingress endpoints | Supported: every advertised endpoint receives every email (fan-out). Per-endpoint size guard — endpoints whose `max_message_size_bytes` the message exceeds are skipped; permanent reject only when no endpoint can accept. Idempotency (Message-ID upsert) makes partial-failure retries safe. Recipient-based routing remains future work. |
